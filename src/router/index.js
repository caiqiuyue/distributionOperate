...
router.beforeEach((to, from, next) => {
  ...
  Util.title(title) // translate zh <=> en
  if (!Cookie.get('user') && to.name !== 'login') {
    ...
    if ((Cookie.get('user')) && store.getters['user/role'].length === 0) { // 已经登录且角色已经分配
      ...
      store.commit('user/USER_INFO', localRole) // vuex管理role
      store.dispatch('permiss/setFilterRoutes', localRole).then((res) => { // 根据role过滤路由
        console.log('filterRoutes', res)
